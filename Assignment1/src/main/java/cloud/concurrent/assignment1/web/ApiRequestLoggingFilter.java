...
@Component
// HIGHEST_PRECEDENCE runs this filter before other application filters, even a request rejected during shutdown still receives an ID and completion log
...
public class ApiRequestLoggingFilter extends OncePerRequestFilter {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        ...
        // startsWith checks the path prefix. ! reverses result bc the inherited method asks when the filter should NOT run.
    ...
    protected void doFilterInternal(
        ...
        // nanoTime = measuring elapsed time rather than displaying a date
        long startNanoseconds = System.nanoTime();
        response.setHeader(REQUEST_ID_HEADER, requestId);

        // MDC attaches the same identifier to any log produced while this request is handled
        // try-with-resources auto closes MDCCloseable and removes the temporary value afterward, preventing one concurrent request's ID leaking into another
        try (MDC.MDCCloseable ignored = MDC.putCloseable("requestId", requestId)) {
            filterChain.doFilter(request, response);
        } finally {
            // finally runs for BOTH successful and failed requests, timing is always logged
