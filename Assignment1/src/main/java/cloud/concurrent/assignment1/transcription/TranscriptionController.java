...
public class TranscriptionController {
    ...
    // static = one class-level value is shared by every controller object
    // final = the reference cannot be reassigned after it is initialised
    private static final int MAX_RECORDING_BYTES = 25 * 1024 * 1024; // 25 MiB (26,214,400 bytes)
    // Map.ofEntries creates an immutable MIME-type allow-list. cannot be changed accidentally while concurrent request threads are reading from it
    ...
    public TranscriptionController(
        ...
        // Spring supplies both dependencies. Depending on the interface lets the default OpenAI service be replaced by the local stub profile or a test double.
     ...
     * backend creates the multipart body required by OpenAI after validating the recording
     */
    // This method RECIEVES POST /api/v1/record/upload
    @PostMapping("/upload")
    public TranscriptionResponse uploadRecording(
            // places the raw uploaded bytes in this array required=false lets our validation return a controlled error when the body is missing.
            @RequestBody(required = false) byte[] audioBytes,
            // reads the browser's Content-Type header in the same way.
        ...
        try {
            // call blocks while the cloud provider responds. 
            // Spring's virtual-thread configuration allows many such blocking requests without one platform thread each
            ...
            // shared statistics must be balanced on both success and failure paths
    ...
    // VALIDATION - private=  internal controller detail, not another HTTP endpoint
    private AudioRecording validate(byte[] audioBytes, String contentType) {
        // Short-circuiting prevents .length from being read when the array is null
        ...
        // split(";", 2)[0] keeps the part before the first semicolon [0] accesses the first item in the resulting String array.
