 ...
 *
 * <p>{@code String text} stores the transcript. The two {@code long} values
 * store whole-number token counts. {@code long} supports much larger totals
 * than {@code int}, which is useful when usage is accumulated across requests.</p>
 */
// A record is suitable here because this class only carries result data.
